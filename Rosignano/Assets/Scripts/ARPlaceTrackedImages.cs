...
public class ARPlaceTrackedImages : MonoBehaviour
{
    ...
    private void OnTrackedImagesChanged(ARTrackedImagesChangedEventArgs eventArgs) 
    {
            ...
            { 
                ...
                { 
                    ...
                    { 
                        ...
                        // newPrefab.transform.SetParent(trackedImage.transform, true);
            ...
            {  
                if(app.GetComponent<Calibration>().gotBoth){
                    _instantiatedPrefabs[trackedImage.referenceImage.name].SetActive(false);
                }else{
                    _instantiatedPrefabs[trackedImage.referenceImage.name].SetActive(trackedImage.trackingState == TrackingState.Tracking);
                    if(trackedImage.trackingState == TrackingState.Tracking){
                        if(dropDown.value == 0)
                            _instantiatedPrefabs[trackedImage.referenceImage.name].transform.GetChild(0).GetChild(0).GetChild(0).GetComponent<TextMeshProUGUI>().text = "PREMI QUI";
                        else _instantiatedPrefabs[trackedImage.referenceImage.name].transform.GetChild(0).GetChild(0).GetChild(0).GetComponent<TextMeshProUGUI>().text = "TAP HERE";
